...
class GeneticAlgo:
    ...
    def run_tetris_bot(self, chromosome, limit=True):
        ...
        while not done:
            ...
            # Record game statistics after episode ends
        Model_Outputs.record_game_statistics(
            heuristic=chromosome,
            high_score=self.env.high_score,
            lines_cleared = self.env.lines_cleared,
            tot_lines_cleared = self.env.total_lines_cleared,
            tetrises=self.env.tetris_count,
            game_overs = self.env.game_overs
        )
    ...
    def run_genetic_algorithm(self):
        ...
        save_heuristic(best_individual)
        return best_individual, best_fitness
            
        
#Total Dones = generations × population_size × num_simulations
#simulations - Update Simulations on line 29
...
#Run Genetic algorithm without given heuristic = genetic_algo.run_genetic_algorithm()
#genetic_algo.run_genetic_algorithm()

# Run genetic algorithm with specified heuristic
